...
//  LearnC
//
//  Created by javalong on 2017/6/6.
...
int main(int argc, const char * argv[])
{
    ...
    if( (sockfd_ = socket(AF_INET,SOCK_DGRAM /*SOCK_RAW*/, protocol->p_proto) )<0)
    {
        perror("socket error");
        exit(1);
    }
    ...
    if( (inaddr = inet_addr(s) ) == INADDR_NONE)
    {
        if((host = gethostbyname(s) ) == NULL) /*是主机名*/
        {
